            ...
            <div className="max-w-7xl mx-auto px-3 md:px-6">
                ...
                {displayDeals.length > 0 ? (
                        ...
                        {displayDeals.map((deal) => (
                            <div key={deal.id} className="group bg-white rounded-xl md:rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col sm:flex-row">
                                <div className="sm:w-2/5 aspect-[16/9] sm:aspect-auto relative overflow-hidden bg-gray-100">
                                    <Image src={deal.imageUrl || '/no-image.svg'} alt={deal.title} fill unoptimized className="object-cover object-center group-hover:scale-105 transition-transform duration-500" />
                                    <div className="absolute top-2 left-2 md:top-4 md:left-4 z-10">
                                        <span className="bg-red-500 text-white text-[8px] md:text-[10px] font-bold px-2 md:px-3 py-0.5 md:py-1 rounded-full uppercase tracking-wider shadow-md">{deal.badge || 'Deal'}</span>
                                    </div>
                                </div>
                    ...
                    <div className="rounded-2xl border border-dashed border-gray-200 bg-white p-8 text-center text-sm text-gray-500">
                        Best deals are not available right now.
                    </div>
                )}
            </div>
        </section>
