...
export function AuthProvider({ children }) {
    ...
    useEffect(() => {
        try {
            const storedToken = localStorage.getItem("sevilla_token");
            const storedUser = localStorage.getItem("sevilla_user");
            ...
            localStorage.removeItem("sevilla_token");
            localStorage.removeItem("sevilla_user");
        } finally {
            setLoading(false);
        }
    }, []);
    ...
    const persistAuth = useCallback((newToken, newUser) => {
        try {
            localStorage.setItem("sevilla_token", newToken);
            localStorage.setItem("sevilla_user", JSON.stringify(newUser));
        } catch (error) {
    ...
    const updateProfile = useCallback(async (profileData) => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setUser(updatedUser);
                try {
                    localStorage.setItem("sevilla_user", JSON.stringify(updatedUser));
                } catch (e) {
