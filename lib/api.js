...
export async function getSlidersFromServer() {
    const sliderId = process.env.NEXT_PUBLIC_SLIDER_ID || process.env.NEXT_PUBLIC_USER_ID;
    const hasSlides = (payload) => {
        if (Array.isArray(payload?.sliders) && payload.sliders.length > 0) return true;
        if (Array.isArray(payload?.data) && payload.data.length > 0) return true;
        return false;
    };

    // Primary: matches backend shape like `/api/get-sliders/26`
    const primaryUrl = `${process.env.NEXT_PUBLIC_API}/get-sliders/${sliderId}`;
    const primaryRes = await fetch(primaryUrl, { cache: "no-store" });
    if (primaryRes.ok) {
        const primaryJson = await primaryRes.json();
        if (hasSlides(primaryJson)) return primaryJson;
    }

    // Fallback: older/public endpoint (keep for backward compatibility)
    const fallbackUrl = `${process.env.NEXT_PUBLIC_API}/public/sliders/${process.env.NEXT_PUBLIC_USER_ID}`;
    const fallbackRes = await fetch(fallbackUrl, { cache: "no-store" });
    if (!fallbackRes.ok) {
        throw new Error(
            `Failed to fetch sliders. Primary ${primaryRes.status}, fallback ${fallbackRes.status}`
        );
    }
    const fallbackJson = await fallbackRes.json();
    if (hasSlides(fallbackJson)) return fallbackJson;

    // Return fallback payload even if empty so caller can decide UI state.
    return fallbackJson;
}

/**
 * Fetch banners from server (with caching)
 * Uses /get-banners/:user_id as primary source.
 * @returns {Promise<Object>} Banners data
 */
export async function getBannerFromServer() {
    return getCachedOrFetch('banners', async () => {
        const primaryUrl = `${process.env.NEXT_PUBLIC_API}/get-banners/${process.env.NEXT_PUBLIC_USER_ID}`;
        const res = await fetch(primaryUrl, { next: { revalidate: 300 } });
        if (res.ok) {
            return res.json();
        }

        // Fallback to older public endpoint if available
        const fallbackUrl = `${process.env.NEXT_PUBLIC_API}/public/banners/${process.env.NEXT_PUBLIC_USER_ID}`;
...
export async function getBestDealsFromServer() {
    return getCachedOrFetch('best_deals', async () => {
        ...
        if (!res.ok) throw new Error("Failed to fetch best deals");
        return res.json();
    });
}
...
/**
 * Fetch latest ecommerce offers (with caching)
 * @returns {Promise<Object>} Special offers data
 */
export async function getOffersFromServer() {
    return getCachedOrFetch('offers', async () => {
        const res = await fetch(
